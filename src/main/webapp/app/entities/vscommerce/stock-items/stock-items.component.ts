...
export class StockItemsComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.stockItemsService
      ...
      .subscribe(
        (res: HttpResponse<IStockItems[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInStockItems(): void {
    this.eventSubscriber = this.eventManager.subscribe('stockItemsListModification', () => this.loadPage());
  }

  delete(stockItems: IStockItems): void {
  ...
  protected onSuccess(data: IStockItems[] | null, headers: HttpHeaders, page: number): void {
