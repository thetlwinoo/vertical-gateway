import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { ICards, Cards } from 'app/shared/model/vscommerce/cards.model';
import { CardsService } from './cards.service';
import { CardsComponent } from './cards.component';
import { CardsDetailComponent } from './cards-detail.component';
import { CardsUpdateComponent } from './cards-update.component';

@Injectable({ providedIn: 'root' })
export class CardsResolve implements Resolve<ICards> {
  constructor(private service: CardsService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ICards> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((cards: HttpResponse<Cards>) => {
          if (cards.body) {
            return of(cards.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new Cards());
  }
}

export const cardsRoute: Routes = [
  {
    path: '',
    component: CardsComponent,
    data: {
      authorities: [Authority.USER],
      pageTitle: 'gatewayApp.vscommerceCards.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/view',
    component: CardsDetailComponent,
    resolve: {
      cards: CardsResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'gatewayApp.vscommerceCards.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: 'new',
    component: CardsUpdateComponent,
    resolve: {
      cards: CardsResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'gatewayApp.vscommerceCards.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/edit',
    component: CardsUpdateComponent,
    resolve: {
      cards: CardsResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'gatewayApp.vscommerceCards.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
];
