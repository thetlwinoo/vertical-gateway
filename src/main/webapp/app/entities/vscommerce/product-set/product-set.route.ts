...
export class ProductSetResolve implements Resolve<IProductSet> {
  constructor(private service: ProductSetService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IProductSet> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((productSet: HttpResponse<ProductSet>) => {
          if (productSet.body) {
            return of(productSet.body);
          } else {
...
export const productSetRoute: Routes = [
  {
    ...
    data: {
      ...
      pageTitle: 'gatewayApp.vscommerceProductSet.home.title',
    },
