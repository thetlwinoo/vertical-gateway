...
export class VehicleTemperaturesResolve implements Resolve<IVehicleTemperatures> {
  constructor(private service: VehicleTemperaturesService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IVehicleTemperatures> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((vehicleTemperatures: HttpResponse<VehicleTemperatures>) => {
          if (vehicleTemperatures.body) {
            return of(vehicleTemperatures.body);
          } else {
...
export const vehicleTemperaturesRoute: Routes = [
  {
    ...
    data: {
      ...
      pageTitle: 'gatewayApp.vscommerceVehicleTemperatures.home.title',
    },
